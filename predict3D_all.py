...
from dataset import PET_images_3D, PET_images_3D_cut, RGB_images_3D_cut, RGB_images_3D_cut_yf_ff
...
def main():
    ...
    # 得到数据集的路径
    image_path = '/home/Shujy/oral_cancer_dataset/imgs'
    ...
    # 消融实验
    model_path = '/home/Shujy/pth/M3D_ResNet/all4/5_block'
    result_path = '/home/Shujy/pth/M3D_ResNet/all4/result_5b.txt'
    model_name = 'M3D_ResNet.pth'
    ...
    # for pth_fn in models:
        ...
        model = M3D_ResNet(BasicBlock, [2, 2, 2, 2],block_num=5,shortcut_type='B',no_cuda=False,num_classes=2,include_top=True)
        model.to(device)
        weights_path = os.path.join(pth_fn,model_name)
        # 载入模型权重
    ...
    s_num = 6
    avg_auc = sum(all_auc[0:s_num])/s_num
    avg_acc = sum(all_acc[0:s_num])/s_num
    avg_jql = sum(all_jql[0:s_num])/s_num
    avg_zhl = sum(all_zhl[0:s_num])/s_num
    avg_mcc = sum(all_mcc[0:s_num])/s_num
    print('avg_acc:{},avg_jql:{},avg_zhl:{},avg_auc:{}'.format(avg_acc,avg_jql,avg_zhl,avg_auc))
    std_acc = sum([(i-avg_acc)**2 for i in all_acc[0:s_num]])/s_num
    std_jql = sum([(i-avg_jql)**2 for i in all_jql[0:s_num]])/s_num
    std_zhl = sum([(i-avg_zhl)**2 for i in all_zhl[0:s_num]])/s_num
    std_mcc = sum([(i-avg_mcc)**2 for i in all_mcc[0:s_num]])/s_num
    std_auc = sum([(i-avg_auc)**2 for i in all_auc[0:s_num]])/s_num
